...
import "./interfaces/IsPOZ.sol";
import "./interfaces/IBondingCalculator.sol";
import "./interfaces/ITreasury.sol";

import "./types/PozzlePlanetAccessControlled.sol";

contract PozzlePlanetTreasury is PozzlePlanetAccessControlled, ITreasury {
    /* ========== DEPENDENCIES ========== */
    ...
    constructor(
        ...
        require(_ohm != address(0), "Zero address: POZ");
        POZ = IPOZ(_ohm);
    ...
    /**
     * @notice allow approved address to deposit an asset for POZ
    ...
    function deposit(
        ...
        // mint POZ needed and store amount of rewards for distribution
        send_ = value.sub(_profit);
        POZ.mint(msg.sender, send_);
    ...
    /**
     * @notice allow approved address to burn POZ for reserves
     * @param _amount uint256
     * @param _token address
     */
    ...
    /**
     * @notice mint new POZ using excess reserves
    ...
    function mint(address _recipient, uint256 _amount) external override {
        ...
        POZ.mint(_recipient, _amount);
        emit Minted(msg.sender, _recipient, _amount);
    }

    /**
     * DEBT: The debt functions allow approved addresses to borrow treasury assets
     * or POZ from the treasury, using sPOZ as collateral. This might allow an
     * sPOZ holder to provide POZ liquidity without taking on the opportunity cost
    ...
    function incurDebt(uint256 _amount, address _token) external override {
        uint256 value;
        if (_token == address(POZ)) {
            require(permissions[STATUS.POZDEBTOR][msg.sender], notApproved);
            value = _amount;
        } else {
        ...
        sPOZ.changeDebt(value, msg.sender, true);
        require(
            sPOZ.debtBalances(msg.sender) <= debtLimit[msg.sender],
            "Treasury: exceeds limit"
        );
        totalDebt = totalDebt.add(value);

        if (_token == address(POZ)) {
            POZ.mint(msg.sender, value);
        ...
        override
    {
        ...
        IERC20(_token).safeTransferFrom(msg.sender, address(this), _amount);
        uint256 value = tokenValue(_token, _amount);
        sPOZ.changeDebt(value, msg.sender, false);
    ...
    /**
     * @notice allow approved address to repay borrowed reserves with POZ
     * @param _amount uint256
     */
    function repayDebtWithPOZ(uint256 _amount) external {
        require(
            permissions[STATUS.RESERVEDEBTOR][msg.sender] ||
                permissions[STATUS.POZDEBTOR][msg.sender],
            notApproved
        );
        POZ.burnFrom(msg.sender, _amount);
        sPOZ.changeDebt(_amount, msg.sender, false);
        totalDebt = totalDebt.sub(_amount);
        ohmDebt = ohmDebt.sub(_amount);
        emit RepayDebt(msg.sender, address(POZ), _amount, _amount);
    }
    ...
    function enable(
        ...
        if (_status == STATUS.SPOZ) {
            ...
            if (!registered) {
                ...
                if (
                    _status == STATUS.LIQUIDITYTOKEN ||
                    _status == STATUS.RESERVETOKEN
                ) {
    ...
    function queueTimelock(
        ...
        if (
            _status == STATUS.RESERVEMANAGER ||
            _status == STATUS.LIQUIDITYMANAGER
        ) {
    ...
    function execute(uint256 _index) external {
        ...
        if (info.managing == STATUS.SPOZ) {
            // 9
            sPOZ = IsPOZ(info.toPermit);
        } else {
    ...
    function excessReserves() public view override returns (uint256) {
        return totalReserves.sub(POZ.totalSupply().sub(totalDebt));
    }

    /**
     * @notice returns POZ valuation of asset
     * @param _token address
     * @param _amount uint256
    ...
    {
        value_ = _amount.mul(10**IERC20Metadata(address(POZ)).decimals()).div(
    ...
    function baseSupply() external view override returns (uint256) {
        return POZ.totalSupply() - ohmDebt;
    }
}
